...
from datasets.segmentation.pascal_voc_augmented.dataset import PascalVocAugmentedSegmentation


class PascalVocAugmentedSegmentationDataLoader(AbstractSegmentationDataLoader):
    def __init__(
            self,
            data_dir: str,
        ...
        super(PascalVocAugmentedSegmentationDataLoader, self).__init__(data_dir, train_batch_size, test_batch_size,
                                                                       image_size, data_idxs)
        ...
        super(PascalVocAugmentedSegmentationDataLoader, self)._data_transforms(**data_transform_args)

    def _get_centralized_data_loader(self, data_idxs: Optional[List[int]] = None) -> Any:
        train_dataset = PascalVocAugmentedSegmentation(self.data_dir, split='train', transform=self.train_transform,
                                                       data_idxs=data_idxs)
        val_dataset = PascalVocAugmentedSegmentation(self.data_dir, split='val', transform=self.val_transform)
