...
class CocoSegmentation(torch.utils.data.Dataset):
    ...
    def __init__(self,
                 ...
                 split: str = 'train',
                 categories: str = None,
                 data_idxs: str = None) -> None:
        if categories is None:
            categories = ['__background__', 'airplane', 'bicycle', 'bird', 'boat', 'bottle', 'bus', 'car', 'cat',
                          'chair', 'cow', 'dining table', 'dog', 'horse', 'motorcycle', 'person', 'potted plant',
                          'sheep', 'sofa', 'tv', 'train']
        self.root_dir = Path('{}/{}'.format(root_dir, year))
        self.data_idxs = data_idxs
        self.annotations_zip_path = Path('{}/annotations_trainval{}.zip'.format(self.root_dir, year))
        self.train_zip_path = Path('{}/train{}.zip'.format(self.root_dir, year))
        self.val_zip_path = Path('{}/val{}.zip'.format(self.root_dir, year))
        self.test_zip_path = Path('{}/test{}.zip'.format(self.root_dir, year))
        self.annotations_path = Path('{}/annotations'.format(self.root_dir))
        self.images_path = Path('{}/{}{}'.format(self.root_dir, split, year))
        self.instances_path = Path('{}/annotations/instances_{}{}.json'.format(self.root_dir, split, year))
    ...
    def __download_dataset(self, year: int, split: str) -> None:
...
if __name__ == '__main__':
    root_dir = './data/coco'
    train_data = CocoSegmentation(root_dir=root_dir)
    val_Data = CocoSegmentation(root_dir=root_dir, split='val')
